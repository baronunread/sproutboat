...
type Authorization = {
  ...
  device_hash: string;
  user_id: string | null;
  state: "pending" | "approved" | "issuing" | "issued";
  expires_at: string;
};
...
function db(): Database {
  if (!database) {
    ...
    database.run(
      "CREATE TABLE IF NOT EXISTS cli_authorizations (user_code TEXT PRIMARY KEY, device_hash TEXT NOT NULL UNIQUE, user_id TEXT, state TEXT NOT NULL, expires_at TEXT NOT NULL, token_issued_at TEXT)",
    );
  }
  return database;
}
...
function active(record: Authorization): boolean {
...
export async function createCliAuthorization(): Promise<Response> {
  for (let attempt = 0; attempt < 5; attempt++) {
    ...
    try {
      db()
        .query(
          "INSERT INTO cli_authorizations (user_code, device_hash, user_id, state, expires_at) VALUES (?, ?, NULL, 'pending', ?)",
        )
        .run(code, hash(deviceCode), expiresAt);
...
export async function approveCliAuthorization(request: Request, code: string): Promise<Response> {
  ...
  if (!actor || actor.authentication !== "session")
    return Response.json({ error: "sign in and reserve a namespace before approving a CLI request" }, { status: 401 });
  // SAFETY: the query selects the Authorization columns from the table created in db().
  const record = db()
    .query("SELECT user_code, device_hash, user_id, state, expires_at FROM cli_authorizations WHERE user_code = ?")
    .get(code) as Authorization | null;
  if (!record || !active(record))
    return Response.json({ error: "CLI request has expired or is invalid" }, { status: 404 });
  if (record.state !== "pending") return Response.json({ error: "CLI request was already handled" }, { status: 409 });
  db()
    .query("UPDATE cli_authorizations SET user_id = ?, state = 'approved' WHERE user_code = ? AND state = 'pending'")
    .run(actor.id, code);
...
export async function exchangeCliAuthorization(deviceCode: string | undefined): Promise<Response> {
  ...
  // SAFETY: the query selects the Authorization columns from the table created in db().
  const record = db()
    .query("SELECT user_code, device_hash, user_id, state, expires_at FROM cli_authorizations WHERE device_hash = ?")
    .get(hash(deviceCode)) as Authorization | null;
  if (!record || !active(record))
    return Response.json({ error: "CLI request has expired or is invalid" }, { status: 404 });
  if (record.state === "pending") return Response.json({ error: "authorization pending" }, { status: 428 });
  if (record.state !== "approved" || !record.user_id)
    return Response.json({ error: "CLI request was already used" }, { status: 410 });
  const claimed = db()
    .query("UPDATE cli_authorizations SET state = 'issuing' WHERE user_code = ? AND state = 'approved'")
    .run(record.user_code);
  ...
  try {
    ...
    db()
      .query("UPDATE cli_authorizations SET state = 'issued', token_issued_at = ? WHERE user_code = ?")
      .run(new Date().toISOString(), record.user_code);
    return Response.json({ token: key });
  } catch (error) {
    db()
      .query("UPDATE cli_authorizations SET state = 'approved' WHERE user_code = ? AND state = 'issuing'")
      .run(record.user_code);
    return Response.json(
      { error: error instanceof Error ? error.message : "unable to create CLI key" },
      { status: 503 },
    );
  }
}
...
async function sessionUser(request: Request): Promise<{ id: string } | Response> {
  ...
  if (actor.authentication !== "session")
    return Response.json({ error: "manage CLI credentials from the dashboard, not a CLI token" }, { status: 403 });
  return { id: actor.id };
}
