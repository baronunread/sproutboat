...
import { stampVersion } from "./version";
...
const server = Bun.serve({
  ...
  hostname,
  async fetch(request) {
    // Every API response advertises this control plane's version and the oldest
    // CLI it supports, so a skewed CLI can say "upgrade" instead of failing on a
    // route that moved. One place, so no handler has to remember.
    return stampVersion(await handle(request), new URL(request.url).pathname);
  },
});
