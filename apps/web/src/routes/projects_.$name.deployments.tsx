...
import { Button, Panel, PanelHeading, RECORD_TITLE, RecordList, RecordRow, SelectField, Status, TextField } from "../components";
    ...
    <Panel variant="bare">
      ...
      <div className="mt-5 mb-3 flex flex-wrap items-end gap-2.5">
        <SelectField label="State" value={status} options={STATE_OPTIONS}
          onValueChange={(value) => { setStatus(value); setShown(PAGE); }} />
        <TextField label="Search" type="search" fieldClassName="min-w-0 flex-[1_1_16rem]" placeholder="Match version id, artifact digest or hostname"
          value={query} onChange={(event) => { setQuery(event.target.value); setShown(PAGE); }} />
      </div>

      {matching.length === 0 ? (
        <p className="px-5 py-12 text-center text-[0.84rem] leading-relaxed text-muted-foreground group-[.is-padded]/panel:px-0 group-[.is-padded]/panel:py-5 group-[.is-padded]/panel:text-start [&_code]:text-foreground">No versions match these filters.</p>
      ) : (
        <>
          <RecordList aria-label="Versions">
            {visible.map((deployment) => (
              <RecordRow key={deployment.id}>
                <div>
                  <Link className={RECORD_TITLE} to="/projects/$name/deployments/$id" params={{ name, id: deployment.id }}>
