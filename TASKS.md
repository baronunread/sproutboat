...
### T1. Corpus
Create `corpus/` with 30 single-file handlers using the frozen contract
...
**Check:** every file runs under `bun run tools/refserve.ts corpus/<f>.js` and
...
`tools/shim.js` is prepended/wrapped around each corpus file so the compiled
...
`tools/diff.ts`: for each corpus file — run via refserve/Bun AND via compiled
...
**Check:** `bun tools/diff.ts` produces report.json covering all corpus files
...
**Check:** `platform deploy corpus/hello.js` → prints live URL; curl works.
...
**Check:** a corpus handler still works jailed; a handler attempting to read
